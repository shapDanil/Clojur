...
(defn add-head [set symbols]                                ;тепер, работает
  (if (empty? symbols)
    ()
    (let [foo (add-head set (rest symbols)) firstSym (first symbols)]
       (if (not= (first set) firstSym)
         (cons (cons firstSym set) foo)
         foo ))))
